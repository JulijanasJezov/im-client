...
public class Connection implements Runnable {
	...
	private void validateMessage(String message) {
		...
		if(message.length() < 4 || message.trim().equals("")){
			...
			switch(message.substring(0,4)){
				...
				case "IDEN":
					if(doesMessageExist(message)) {
						iden(message.substring(5));
					} else {
						sendOverConnection("BAD missing username");
					}
					break;
				...
				case "HAIL":
					if(doesMessageExist(message)) {
						...
						sendOverConnection("BAD missing message");
					}
					break;
	...
	private boolean doesMessageExist(String message) {
		if (message.length() < 6 || message.substring(5).trim().equals(""))
			return false;
		else {
			return true;
		}
	...
	private void iden(String message) {
		switch(state) {
			...
			case STATE_UNREGISTERED:
				...
				if (username.equals("")) {
					sendOverConnection("BAD username cannot start with the space");
				} else {
		...
		switch(state) {
			case STATE_REGISTERED:
				...
				if(message.contains(" ")) {
					...
					if (pm.trim().equals("")) {
						sendOverConnection("BAD missing message");
					} else if (serverReference.sendPrivateMessage("PM from " + username + ":" + pm, user)){
